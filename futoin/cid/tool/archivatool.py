...
<propfind xmlns="DAV:">
  <prop>
    ...
    <iscollection/>
        ...
        for response in apidom.getElementsByTagNameNS('*', 'response'):
            if response.getElementsByTagNameNS('*', 'iscollection')[0].firstChild.nodeValue == '0':
                name = response.getElementsByTagNameNS('*', 'displayname')[0].firstChild.nodeValue
    ...
    def rmsPoolCreate( self, config, rms_pool ):
        res = self._callArchiva(
            config,
            'GET', '/restServices/archivaServices/managedRepositoriesService/getManagedRepository/{0}'.format(rms_pool),
    ...
    def rmsGetHash(self, config, rms_pool, package, hash_type ):
        ...
        res = self._callArchiva(
            config,
            'GET', '/repository/{0}/{1}.{2}'.format(rms_pool, package, hash_type)
        )
        res.raise_for_status()
