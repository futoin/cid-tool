...
"""
    ...
    def rmsUpload( self, config, rms_pool, package_list ):
        for package in package_list:
            ...
            path = '/content/repositories/{0}/{1}'.format(rms_pool, package_basename)
    ...
    def rmsGetList( self, config, rms_pool, package_hint ):
        ...
        res = self._callNexus(
            config,
            'GET', '/service/local/repositories/{0}/content/{1}'.format(pool, path),
    ...
    def rmsGetHash(self, config, rms_pool, package, hash_type ):
        ...
        res = self._callNexus(
            config,
            'GET', '/service/local/repositories/{0}/content/{1}?describe=info'.format(pool, path),
            headers = { 'Accept' : 'application/json' },
        )
        res.raise_for_status()
        res = res.json()
