...
"""    
    ...
    def vcsGetRefRevision( self, config, vcs_cache_dir, branch ) :
        ...
        res = self._callVCSSVN( config, [ 'info', svn_repo_path, '--xml' ], verbose=False )
    ...
    def vcsMerge( self, config, vcs_ref, cleanup ):
        ...
        self._callVCSSVN( config, ['merge', '--accept', 'postpone', svn_repo_path ] )
    ...
    def vcsDelete( self, config, vcs_cache_dir, vcs_ref ):
        ...
        self._callVCSSVN( config, ['remove', '-m', 'CID delete ' + vcs_ref, svn_repo_path ] )
    ...
    def _callSVN( self, config, repo_key, args, **kwargs ):
        ...
        if rms_repo.startswith('svn+ssh://'):
            ...
            ssh_opts = '-o BatchMode=yes -o StrictHostKeyChecking={0}'.format(env['sshStrictHostKeyChecking'])
            opts += [
                '--config-option',
                'config:tunnels:ssh=$SVN_SSH ssh -q -p {0} {1}'.format(ssh_port, ssh_opts)
            ]
